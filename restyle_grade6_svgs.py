#!/usr/bin/env python3
"""
Restyle all grade 6 SVG images with subject-specific color schemes.
...
IMG_DIR = f"{BASE}/public/images/lessons/grade6"
...
def get_subject_decorative(subject, colors):
  ...
  <rect x="460" y="115" width="4" height="25" fill="{corners[2]}" opacity="0.08" transform="rotate(-20,462,128)"/>'''
        ...
        # Crafts: weaving/cross-hatch pattern
        return f'''<line x1="435" y1="110" x2="475" y2="145" stroke="{primary}" stroke-width="1" opacity="0.1"/>
  <line x1="475" y1="110" x2="435" y2="145" stroke="{corners[2]}" stroke-width="1" opacity="0.08"/>
  <line x1="455" y1="105" x2="455" y2="150" stroke="{corners[3]}" stroke-width="0.5" opacity="0.1"/>'''
...
def restyle_svg(content, subject, colors):
    ...
    # 2. Replace frame and template colors (first ~22 lines)
    lines = content.split('\n')
    new_lines = []
...
def main():
    ...
    for subject in sorted(os.listdir(IMG_DIR)):
        ...
        for svg_file in sorted(svg_files):
            ...
            # Check if this SVG needs restyling
