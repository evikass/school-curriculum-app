...
const NAMES = { tech: 'Технология', projects: 'Проектная деятельность', world: 'Окружающий мир' };
const name = NAMES[SUBJECT];
if (!name) { console.error('Usage: node gen.mjs <tech|projects|world>'); process.exit(1); }
...
async function main() {
  ...
  const data = JSON.parse(fs.readFileSync(`${DIR}/${SUBJECT}.json`, 'utf-8'));
    ...
    for (const lesson of topic.lessons) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          if (!ok(content) && attempt < 2) { console.log(`  retry ${attempt+1}: desc=${content.description?.length}`); await sleep(300); continue; }
          break;
        } catch(e) { console.log(`  err ${attempt+1}: ${e.message.substring(0,60)}`); await sleep(300); }
      }
  ...
  fs.writeFileSync(`${DIR}/${SUBJECT}.json`, JSON.stringify(data, null, 2));
