...
const GAME_BUTTONS: { id: MiniGameType; icon: React.ReactNode; label: string; color: string }[] = [
  ...
  { id: 'chemistry', icon: <FlaskConical className="w-3 h-3 inline mr-1" />, label: 'Хим', color: 'lime' },
  { id: 'biology', icon: <Microscope className="w-3 h-3 inline mr-1" />, label: 'Био', color: 'green' },
  { id: 'astronomy', icon: <Star className="w-3 h-3 inline mr-1" />, label: 'Космос', color: 'indigo' },
  { id: 'logic', icon: <Brain className="w-3 h-3 inline mr-1" />, label: 'Логика', color: 'purple' },
  { id: 'wordprob', icon: <FileText className="w-3 h-3 inline mr-1" />, label: 'Задачи', color: 'amber' },
  { id: 'reading', icon: <BookOpen className="w-3 h-3 inline mr-1" />, label: 'Чтение', color: 'teal' },
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <header className="text-center mb-6">
          ...
          <div className="flex justify-center gap-2 mt-3 flex-wrap">
            <span className="text-green-400 text-xs bg-green-400/20 px-3 py-1 rounded-full">v2.79</span>
