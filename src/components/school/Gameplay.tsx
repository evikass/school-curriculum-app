...
export default function Gameplay() {
  ...
  const submitOrderAnswer = () => {
    const userAnswer = ((answers[currentIndex] as string) || '').trim()
    // Normalize user answer: remove extra spaces, split by comma
    const userItems = userAnswer.split(',').map(s => s.trim()).filter(s => s)
    const correctItems = Array.isArray(task.correctAnswer) 
      ? task.correctAnswer.map(s => String(s).trim())
      : [String(task.correctAnswer).trim()]
    
    // Check if arrays match (order matters)
    const isCorrect = userItems.length === correctItems.length && 
      userItems.every((item, i) => item === correctItems[i])
      ...
      <div className={`p-8 rounded-3xl bg-gradient-to-br from-gray-800/80 to-gray-900/80 border-4 transition-all duration-300 ${
        ...
        {task?.type === 'order' && task.options && (
          <div className="space-y-4">
            ...
            <input type="text" value={(answers[currentIndex] as string) || ''}
              onChange={(e) => answerState === 'idle' && setAnswers({ ...answers, [currentIndex]: e.target.value })}
              onKeyDown={(e) => e.key === 'Enter' && answerState === 'idle' && submitOrderAnswer()}
            ...
            {answerState === 'idle' && (
              <button onClick={submitOrderAnswer}
                className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-2xl font-bold hover:scale-[1.02] transition-transform">
                <Zap className="w-5 h-5 inline mr-2" />Проверить
              </button>
            )}
            ...
            {answerState !== 'idle' && (
              <div className={`p-4 rounded-xl ${answerState === 'correct' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                {answerState === 'correct' ? '✓ Правильно!' : `✗ Правильный ответ: ${Array.isArray(task.correctAnswer) ? task.correctAnswer.join(', ') : task.correctAnswer}`}
              </div>
            )}
          </div>
        )}
        ...
        {task?.type === 'match' && task.options && task.correctAnswer && (
          <div className="space-y-4">
            ...
            {/* 
              Support two formats:
              1. New format: options = [left1, left2, right1, right2], correctAnswer = ["left→right", ...]
              2. Old format: options = [left1, left2], correctAnswer = [right1, right2] (parallel arrays)
            */}
            {(() => {
              // Ensure correctAnswer is an array of strings
              const correctAnswerArray = Array.isArray(task.correctAnswer) 
                ? (task.correctAnswer as any[]).map(a => String(a || ''))
                : [String(task.correctAnswer || '')]
              
              // Detect format by checking if correctAnswer contains "→"
              const isNewFormat = correctAnswerArray.some(ans => typeof ans === 'string' && ans.includes('→'))
              
              let leftItems: string[]
              let rightItems: string[]
              let correctPairs: Map<string, string>
              
              if (isNewFormat) {
                // New format: split options into left/right halves
                const halfLength = Math.ceil(task.options.length / 2)
                leftItems = task.options.slice(0, halfLength)
                rightItems = task.options.slice(halfLength)
                correctPairs = new Map(
                  correctAnswerArray.map(pair => {
                      ...
                      return (
                        <button key={`right-${index}`}
                          onClick={() => {
                            ...
                            const expectedRight = correctPairs.get(selectedLeft)
                            if (expectedRight === option) {
