      ...
      <div className="w-full animate-slideIn">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-gradient-to-br from-indigo-500/30 via-purple-500/30 to-pink-500/30 
            ...
            {/* Анимированный SVG или изображение урока */}
            <div className="flex justify-center mb-8">
              {selectedLesson.image ? (
                <div className="relative rounded-3xl overflow-hidden border-4 border-purple-400/30 shadow-xl max-w-md">
                  <img 
            ...
            <div className="space-y-0">
              {formatKidText(selectedLesson.theory || selectedLesson.description)}
            </div>

            {/* Ключевые моменты */}
            {selectedLesson.keyPoints && selectedLesson.keyPoints.length > 0 && (
              <div className="mt-8 bg-cyan-500/20 rounded-3xl p-6 border-2 border-cyan-400/30">
                <h4 className="text-xl md:text-2xl font-bold text-cyan-300 mb-4 flex items-center gap-3">
                ...
                <div className="space-y-3">
                  {selectedLesson.keyPoints.map((point, i) => (
                    <div key={i} className="flex items-start gap-4 text-white/90 text-lg">
                      <span className="w-8 h-8 rounded-full bg-cyan-400 flex items-center justify-center text-sm font-bold text-cyan-900 flex-shrink-0 mt-1">
                        {i + 1}
                      </span>
                      <span>{point}</span>
                    </div>
            ...
            {selectedLesson.examples && selectedLesson.examples.length > 0 && (
              <div className="mt-8 bg-purple-500/20 rounded-3xl p-6 border-2 border-purple-400/30">
                <h4 className="text-xl md:text-2xl font-bold text-purple-300 mb-4">
                  📌 Примеры:
                </h4>
                <div className="space-y-2">
                  {selectedLesson.examples.map((example, i) => (
                    <div key={i} className="text-white/90 text-lg pl-4 border-l-4 border-purple-400">
                      {example}
                    </div>
                  ))}
                </div>
            ...
            {selectedLesson.facts && selectedLesson.facts.length > 0 && (
              <div className="mt-8 bg-yellow-500/20 rounded-3xl p-6 border-2 border-yellow-400/30">
                <h4 className="text-xl md:text-2xl font-bold text-yellow-300 mb-4 flex items-center gap-3">
                  <Star className="w-7 h-7" />
                  Интересные факты:
                </h4>
                <div className="space-y-2">
                  {selectedLesson.facts.map((fact, i) => (
                    <div key={i} className="text-white/90 text-lg">
                      ⭐ {fact}
                    </div>
                  ))}
                </div>
