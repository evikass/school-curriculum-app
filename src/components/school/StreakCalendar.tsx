...
export default function StreakCalendar() {
  ...
  useEffect(() => {
    ...
    if (savedProgress) {
      ...
      // Initialize longestStreak with currentStreak if no calendar data
      setLongestStreak(streak)
    }
    
    // Calculate longest streak from data - check for consecutive days
    if (savedStreakData) {
      ...
      const sortedDates = Object.keys(data)
        .filter(date => data[date].hasActivity)
        .sort()
