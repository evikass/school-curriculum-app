import { SubjectData, GameLesson } from '@/data/types'

export const lessons: SubjectData = {
  title: "Иностранный язык",
  icon: "Languages",
  color: "text-pink-400",
  topics: ["My Family and Friends", "Daily Routine", "Hobbies and Interests", "Travelling"],
  detailedTopics: [
    {
      topic: "My Family and Friends",
      lessons: [
        {
          title: "Family Members",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson1.svg",
          description: `## Family Members

:- **Mother / Mom / Mum** — мама
:- **Father / Dad** — папа
:- **Parents** — родители
:- **Sister** — сестра
:- **Brother** — брат
:- **Grandmother / Grandma** — бабушка
:- **Grandfather / Grandpa** — дедушка
:- **Grandparents** — бабушка и дедушка

# Extended Family

:- **Aunt** — тётя
:- **Uncle** — дядя
:- **Cousin** — двоюродный брат/сестра
:- **Nephew** — племянник
:- **Niece** — племянница

## Possessive Adjectives

!! Possessive adjectives show who something belongs to.

:- **I → my** (my family)
:- **You → your** (your brother)
:- **He → his** (his father)
:- **She → her** (her mother)
:- **We → our** (our parents)
:- **They → their** (their children)

> **Examples:**
> - This is my mother. Her name is Anna.
> - I have a sister. Her name is Kate.
> - My uncle lives in Moscow.
> - Their children go to school.

!!! **Key Points:**
- Use "my, your, his, her" for singular
- Use "our, their" for plural
- Possessive adjectives come before nouns

## Questions About Family

> - Do you have any brothers or sisters?
> - How many people are in your family?
> - What does your father do?
> - Who is the oldest in your family?

+ Great job learning family vocabulary! Now you can talk about your relatives in English! 👨‍👩‍👧‍👦`,
          tasks: [
            "Translate: mother, father, sister, brother",
            "Make a sentence with 'my mother'",
            "How do you say 'aunt' and 'uncle' in English?",
            "Ask a question about family",
            "Describe your family using possessive adjectives",
            "Write 3 sentences about your grandparents"
          ],
          examples: ["This is my mother. Her name is Anna. She is a teacher.", "My uncle lives in Moscow. He has two children.", "Their grandparents are from Kazan."],
          keyPoints: [
            "Family members: mother, father, sister, brother, grandmother, grandfather",
            "Possessive adjectives: my, your, his, her, our, their show who owns something",
            "To be (am/is/are) is used to describe people and relationships"
          ]
        },
        {
          title: "Describing People",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson2.svg",
          description: `## Describing People

# Physical Appearance

## Height and Build

:- **tall** — высокий
:- **short** — низкий
:- **medium height** — среднего роста
:- **slim** — стройный
:- **athletic** — атлетический
:- **plump** — полный

## Hair

:- **long / short hair** — длинные / короткие волосы
:- **straight** — прямые
:- **curly** — кудрявые
:- **wavy** — волнистые
:- **fair / blond** — светлые
:- **dark** — тёмные
:- **brown** — каштановые
:- **red / ginger** — рыжие

## Eyes

:- **blue eyes** — голубые глаза
:- **brown eyes** — карие глаза
:- **green eyes** — зелёные глаза
:- **grey eyes** — серые глаза

# Character and Personality

:- **kind** — добрый
:- **friendly** — дружелюбный
:- **clever / smart** — умный
:- **funny** — смешной
:- **serious** — серьёзный
:- **shy** — застенчивый
:- **brave** — храбрый
:- **honest** — честный

## Grammar Structures

!! Use "have got / has got" for physical features:
!! Use "am / is / are" for character traits:

> **Examples:**
> - He has got blue eyes. = He's got blue eyes.
> - She is tall and slim.
> - My friend is kind and funny.
> - Her sister has got long curly hair.
> - He is short with brown eyes.

!!! **Key Points:**
- "Have got" = "have" (British English)
- "Has got" = "has" (for he/she/it)
- Use adjectives after "be" verb

+ Excellent! Now you can describe how people look and their personality! 😊`,
          tasks: [
            "Describe your friend's appearance",
            "How do you say 'he has blue eyes'?",
            "Translate: tall, short, slim",
            "Describe a family member's character",
            "Write 5 sentences about someone's appearance",
            "Use 'have got' in 3 sentences"
          ],
          examples: ["He has got blue eyes and dark hair.", "She is tall and slim. She is very friendly.", "My friend is short with brown eyes and curly hair."],
          keyPoints: [
            "Adjectives describe appearance: tall, short, young, old, blonde, dark-haired",
            "Has got is used to describe physical features and possessions",
            "Questions with Who, What, Where for asking about people"
          ]
        }
      ]
    },
    {
      topic: "Daily Routine",
      lessons: [
        {
          title: "Time and Daily Activities",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson3.svg",
          description: `## Time and Daily Activities

# Telling the Time

## Full Hours

:- **It's 7 o'clock.** — 7 часов.
:- **It's seven a.m.** — 7 утра.
:- **It's seven p.m.** — 7 вечера.

## Minutes

:- **half past seven** — половина восьмого (7:30)
:- **quarter past seven** — четверть восьмого (7:15)
:- **quarter to eight** — без четверти восемь (7:45)
:- **ten past three** — десять минут четвёртого (3:10)
:- **twenty to six** — без двадцати шесть (5:40)

# Daily Activities

:- **wake up** — просыпаться
:- **get up** — вставать
:- **have breakfast** — завтракать
:- **go to school** — идти в школу
:- **have lessons** — иметь уроки
:- **have lunch** — обедать
:- **do homework** — делать домашнее задание
:- **have dinner** — ужинать
:- **watch TV** — смотреть телевизор
:- **go to bed** — ложиться спать

## Present Simple for Routine

!! Present Simple is used for regular actions and routines.

> **Examples:**
> - I wake up at 7 o'clock.
> - She goes to school at 8.
> - We have dinner at 7 p.m.
> - He doesn't watch TV in the morning.
> - Do you do your homework after school?

!!! **Key Points:**
- Add -s/-es for he/she/it
- Use "don't / doesn't" for negatives
- Use "Do / Does" for questions

## Questions About Routine

> - What time do you get up?
> - When do you do your homework?
> - What do you do in the evening?
> - What time does school start?

+ Well done! You can now talk about your daily schedule in English! ⏰`,
          tasks: [
            "How do you say 'half past seven'?",
            "Tell about your morning routine",
            "Translate: wake up, have breakfast, go to bed",
            "Ask a question about daily routine",
            "Write 5 sentences about your day using Present Simple",
            "Describe what time you do different activities"
          ],
          examples: ["I wake up at 7 o clock and have breakfast at 7:30.", "She goes to school at 8 and does her homework in the evening.", "We usually have dinner at 7 p.m."],
          keyPoints: [
            "Time expressions: in the morning, in the afternoon, at night, at weekends",
            "Present Simple for daily routines and regular activities",
            "Adverbs of frequency: always, usually, often, sometimes, never"
          ]
        },
        {
          title: "Days of the Week and Prepositions",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson4.svg",
          description: `## Days of the Week and Prepositions

# Days of the Week

:- **Monday** — понедельник
:- **Tuesday** — вторник
:- **Wednesday** — среда
:- **Thursday** — четверг
:- **Friday** — пятница
:- **Saturday** — суббота
:- **Sunday** — воскресенье

!! Remember: Days of the week start with a capital letter!

# Prepositions of Time

## AT — for exact time

:- **at 7 o'clock** — в 7 часов
:- **at noon** — в полдень
:- **at night** — ночью
:- **at midnight** — в полночь

## ON — for days and dates

:- **on Monday** — в понедельник
:- **on Friday morning** — в пятницу утром
:- **on my birthday** — в мой день рождения
:- **on the 5th of May** — 5 мая

## IN — for months, seasons, parts of day

:- **in September** — в сентябре
:- **in winter** — зимой
:- **in the morning** — утром
:- **in the evening** — вечером
:- **in 2024** — в 2024 году

> **Examples:**
> - I go to school at 8 o'clock on weekdays.
> - We visit our grandparents on Sunday.
> - She reads books in the evening.
> - My birthday is in June.

# Adverbs of Frequency

:- **always (100%)** — всегда
:- **usually (80%)** — обычно
:- **often (60%)** — часто
:- **sometimes (40%)** — иногда
:- **rarely (20%)** — редко
:- **never (0%)** — никогда

!! Adverbs of frequency go BEFORE the main verb!

> **Examples:**
> - I always have breakfast.
> - She never watches TV in the morning.
> - We usually go to the park on Sunday.
> - He often plays football after school.

!!! **Key Points:**
- AT = exact time, ON = days, IN = longer periods
- Adverbs of frequency come before verbs
- "Never" makes the sentence negative

+ Great! You've learned how to talk about when things happen! 📅`,
          tasks: [
            "Name all days of the week in English",
            "When do we use at, on, in?",
            "Make a sentence with 'usually'",
            "Translate: always, sometimes, never",
            "Write sentences using at, on, in correctly",
            "Describe your weekly routine with adverbs of frequency"
          ],
          examples: ["I always do my homework after school.", "She never watches TV in the morning.", "We visit our grandparents on Sunday."],
          keyPoints: [
            "Days of the week: Monday through Sunday, always capitalised in English",
            "Prepositions of time: in (months/years), on (days), at (clock time)",
            "Ordinal numbers: first, second, third for dates"
          ]
        }
      ]
    },
    {
      topic: "Hobbies and Interests",
      lessons: [
        {
          title: "Free Time Activities",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson5.svg",
          description: `## Free Time Activities

# Hobbies

## Sports

:- **play football** — играть в футбол
:- **play basketball** — играть в баскетбол
:- **play tennis** — играть в теннис
:- **play volleyball** — играть в волейбол
:- **swim** — плавать
:- **run** — бегать
:- **ride a bike** — кататься на велосипеде
:- **skate** — кататься на коньках
:- **ski** — кататься на лыжах

## Creative Activities

:- **draw** — рисовать
:- **paint** — писать красками
:- **play the guitar** — играть на гитаре
:- **play the piano** — играть на пианино
:- **sing** — петь
:- **dance** — танцевать
:- **write stories** — писать рассказы

## Other Activities

:- **read books** — читать книги
:- **watch films** — смотреть фильмы
:- **play computer games** — играть в компьютерные игры
:- **listen to music** — слушать музыку
:- **collect stamps** — коллекционировать марки
:- **take photos** — фотографировать
:- **cook** — готовить

# Talking About Hobbies

## Questions

> - What do you like doing?
> - What are your hobbies?
> - Do you play any sports?
> - What do you do in your free time?

## Answers

!! Use "like / love / enjoy + V-ing":

> **Examples:**
> - I like reading books.
> - My hobby is photography.
> - I enjoy playing football.
> - I'm interested in music.
> - She loves dancing.
> - He likes swimming.

## Be Interested In / Be Good At

:- **be interested in** — интересоваться чем-то
:- **be good at** — быть хорошим в чём-то

> **Examples:**
> - I'm interested in art.
> - She's good at playing tennis.
> - He's interested in computers.

!!! **Key Points:**
- After "play" use "the" for instruments (play the guitar)
- After "play" NO article for sports (play football)
- Use -ing form after like/love/enjoy

+ Fantastic! Now you can talk about your hobbies and interests! 🎨⚽`,
          tasks: [
            "Translate hobbies: read, swim, draw",
            "Tell about your interests",
            "How do you ask about hobbies?",
            "Make sentences with 'like' and 'enjoy'",
            "Write about 5 activities you like",
            "Describe your friend's hobbies"
          ],
          examples: ["I like reading books and playing football.", "She is interested in music and dancing.", "He is good at swimming and riding a bike."],
          keyPoints: [
            "Free time vocabulary: reading, swimming, playing sports, watching films",
            "Like + -ing: I like reading, She enjoys swimming",
            "Can/Cannot for abilities: I can play football, He can't swim"
          ]
        },
        {
          title: "Can / Can't for Abilities",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson6.svg",
          description: `## Can / Can't for Abilities

# Modal Verb CAN

:- **can** — мочь, уметь (ability)
:- **can't / cannot** — не мочь, не уметь

!! CAN doesn't change for he/she/it — no -s ending!
!! CAN is followed by the base form of the verb.

# Expressing Ability

## Positive Sentences

> **Examples:**
> - I can swim. — Я умею плавать.
> - She can play the piano. — Она умеет играть на пианино.
> - They can speak English. — Они умеют говорить по-английски.
> - He can ride a bike. — Он умеет кататься на велосипеде.

## Negative Sentences

> **Examples:**
> - He can't drive. — Он не умеет водить.
> - She cannot swim. — Она не умеет плавать.
> - I can't play chess. — Я не умею играть в шахматы.

# Questions with CAN

!! Put CAN at the beginning of the question!

> **Examples:**
> - Can you swim? — Ты умеешь плавать?
> - Can she speak English? — Она умеет говорить по-английски?
> - Can they help us? — Они могут нам помочь?

## Short Answers

:- **Yes, I can.** — Да, умею.
:- **No, I can't.** — Нет, не умею.
:- **Yes, she can.** — Да, она умеет.
:- **No, he can't.** — Нет, он не умеет.

# Abilities Vocabulary

:- **speak English** — говорить по-английски
:- **play chess** — играть в шахматы
:- **cook** — готовить
:- **ride a bike** — кататься на велосипеде
:- **use a computer** — пользоваться компьютером
:- **sing** — петь
:- **dance** — танцевать

# Making Requests

!! CAN is also used for requests and permission!

> **Examples:**
> - Can you help me? — Ты можешь мне помочь?
> - Can I borrow your pen? — Можно одолжить ручку?
> - Can I go out? — Можно мне выйти?

!!! **Key Points:**
- CAN stays the same for all persons
- Use base verb after CAN
- CAN'T = cannot
- Use CAN for ability, requests, permission

+ Excellent work! You can now talk about abilities in English! 💪`,
          tasks: [
            "How do you say 'I can swim'?",
            "Ask a question about ability",
            "Answer briefly: Can you speak English?",
            "Tell what you can and can't do",
            "Make 3 questions using 'Can you...?'",
            "Write about your abilities and talents"
          ],
          examples: ["I can swim, but I cannot drive.", "She can play the piano very well.", "Can you help me with this task?"],
          keyPoints: [
            "Can expresses ability, permission and possibility in English",
            "Can is the same for all persons: I can, he can, we can",
            "Negative: cannot or can't; question: Can you...? — Yes, I can / No, I can't"
          ]
        }
      ]
    },
    {
      topic: "Travelling",
      lessons: [
        {
          title: "At the Airport",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson7.svg",
          description: `## At the Airport

# Airport Vocabulary

## Places at the Airport

:- **airport** — аэропорт
:- **departure lounge** — зал вылета
:- **arrival hall** — зал прилёта
:- **gate** — выход на посадку
:- **baggage claim** — выдача багажа
:- **customs** — таможня
:- **check-in desk** — стойка регистрации
:- **duty-free shop** — магазин беспошлинной торговли
:- **security check** — контроль безопасности

## Things

:- **passport** — паспорт
:- **ticket** — билет
:- **boarding pass** — посадочный талон
:- **luggage / baggage** — багаж
:- **suitcase** — чемодан
:- **hand luggage** — ручная кладь

# Actions at the Airport

:- **check in** — регистрироваться
:- **board the plane** — садиться в самолёт
:- **take off** — взлетать
:- **land** — приземляться
:- **go through security** — проходить контроль
:- **collect luggage** — получать багаж

# Useful Phrases

## At Check-in

> - I'd like a window seat. — Я хотел бы место у окна.
> - I'd like an aisle seat. — Я хотел бы место у прохода.
> - Here is my passport. — Вот мой паспорт.
> - How much luggage can I take? — Сколько багажа я могу взять?

## At the Gate

> - Where is gate 5? — Где выход 5?
> - What time does the flight leave? — Во сколько вылет?
> - Is the flight on time? — Рейс вовремя?

# Airport Announcements

:- **Flight KL123 is now boarding.** — Рейс KL123 объявляет посадку.
:- **Please fasten your seatbelts.** — Пожалуйста, пристегните ремни.
:- **The flight is delayed.** — Рейс задержан.
:- **This is the final call for flight...** — Последний вызов на рейс...

!!! **Key Points:**
- Arrive at the airport 2-3 hours before flight
- Keep your passport and ticket handy
- Check your gate number on the board

+ Great! You're ready to travel by plane in English! ✈️`,
          tasks: [
            "Translate: passport, ticket, luggage",
            "How do you ask 'Where is gate 5?'",
            "What do they say during boarding?",
            "Make a dialogue at the airport",
            "Write 5 useful phrases for travellers",
            "Describe what you do at the airport step by step"
          ],
          examples: ["Here is my passport. I would like a window seat, please.", "Where is gate 5? The flight leaves at 3 p.m.", "I have one suitcase and a hand luggage."],
          keyPoints: [
            "Airport vocabulary: boarding pass, gate, departure, arrival, luggage",
            "Travel phrases: I'd like to book..., Where is gate...?, How long is the flight?",
            "Imperative mood for instructions and requests"
          ]
        },
        {
          title: "Countries and Nationalities",
          image: "/school-curriculum-app/images/lessons/grade6/english/lesson8.svg",
          description: `## Countries and Nationalities

# Countries and Nationalities Table

| Country | Nationality |
|---------|-------------|
| Russia | Russian |
| England / the UK | English / British |
| the USA | American |
| France | French |
| Germany | German |
| Spain | Spanish |
| Italy | Italian |
| China | Chinese |
| Japan | Japanese |
| Australia | Australian |
| Canada | Canadian |

!! Country names start with capital letters!
!! Nationality adjectives also start with capital letters!

# Capital Cities

> **Examples:**
> - Moscow is the capital of Russia.
> - London is the capital of England.
> - Paris is the capital of France.
> - Washington D.C. is the capital of the USA.
> - Tokyo is the capital of Japan.
> - Beijing is the capital of China.

# Asking and Answering About Origin

## Questions

> - Where are you from? — Откуда ты?
> - What nationality are you? — Кто ты по национальности?
> - Where do you come from? — Откуда ты родом?

## Answers

> - I'm from Russia. — Я из России.
> - I'm Russian. — Я русский.
> - I come from Moscow. — Я из Москвы.

# Languages

:- **Russian** — русский язык
:- **English** — английский язык
:- **French** — французский язык
:- **German** — немецкий язык
:- **Spanish** — испанский язык
:- **Chinese** — китайский язык
:- **Japanese** — японский язык

## Speaking Languages

> **Examples:**
> - I speak Russian. — Я говорю по-русски.
> - She speaks English. — Она говорит по-английски.
> - They speak French. — Они говорят по-французски.
> - Do you speak English? — Ты говоришь по-английски?

# The Article "THE"

!! Use "the" with country names that include words like "kingdom", "states", "republic":

> - the United Kingdom (the UK)
> - the United States (the USA)
> - the Russian Federation

!!! **Key Points:**
- Country → Nationality (Russia → Russian)
- Use "I'm from + country" for origin
- Use "I speak + language" for languages
- Remember "the" with UK, USA

+ Wonderful! Now you can talk about countries and nationalities! 🌍`,
          tasks: [
            "Name 5 countries and nationalities",
            "How do you ask 'Where are you from?'",
            "Tell about capital cities",
            "Make a dialogue about your country",
            "Write about 3 countries you'd like to visit",
            "Describe your nationality and languages you speak"
          ],
          examples: ["Moscow is the capital of Russia.", "I am from Russia. I speak Russian.", "She is French. She lives in Paris."],
          keyPoints: [
            "Countries and nationalities: France — French, Japan — Japanese, Russia — Russian",
            "Languages: English, Spanish, Chinese, Arabic, German",
            "To be from / To come from for talking about origin"
          ]
        }
      ]
    }
  ]
}



export const games = [
  {
    title: "Family Members",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "How do you say 'мама' in English?", options: ["Father", "Mother", "Sister", "Brother", "Другой ответ"], correctAnswer: "Mother", hint: "Female parent" },
      { type: 'find', question: "Choose family members:", options: ["Mother", "Teacher", "Father", "Doctor", "Sister", "Driver"], correctAnswer: ["Mother", "Father", "Sister"], hint: "People in a family" },
      { type: 'quiz', question: "What does 'nephew' mean?", options: ["Племянница", "Дядя", "Племянник", "Двоюродный брат", "Другой ответ"], correctAnswer: "Племянник", hint: "Brother's or sister's son" },
      { type: 'quiz', question: "Complete: 'This is ___ mother. ___ name is Anna.'", options: ["my / Her", "I / She", "me / Hers", "my / She", "Другой ответ"], correctAnswer: "my / Her", hint: "Possessive adjectives before nouns" }
    ],
    reward: { stars: 5, message: "Great! You know family vocabulary! 👨‍👩‍👧‍👦" }
  },
  {
    title: "Describing People",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "What does 'slim' mean?", options: ["Высокий", "Стройный", "Полный", "Низкий", "Другой ответ"], correctAnswer: "Стройный", hint: "Thin and attractive" },
      { type: 'quiz', question: "How do you say 'у неё длинные кудрявые волосы'?", options: ["She has got long straight hair", "She has got long curly hair", "She has got short curly hair", "She has got long wavy hair", "Другой ответ"], correctAnswer: "She has got long curly hair", hint: "long + curly" },
      { type: 'quiz', question: "Which structure describes character traits?", options: ["have got", "am / is / are", "there is / there are", "was / were", "Другой ответ"], correctAnswer: "am / is / are", hint: "She is kind and friendly" },
      { type: 'quiz', question: "What does 'brave' mean?", options: ["Застенчивый", "Честный", "Храбрый", "Серьёзный", "Другой ответ"], correctAnswer: "Храбрый", hint: "Not afraid" }
    ],
    reward: { stars: 5, message: "Great job! You can describe people in English! 😊" }
  },
  {
    title: "Time and Daily Activities",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "What time is 'half past seven'?", options: ["7:00", "7:15", "7:30", "7:45", "Другой ответ"], correctAnswer: "7:30", hint: "30 minutes after 7" },
      { type: 'find', question: "Choose daily activities:", options: ["Wake up", "Paint", "Have breakfast", "Sleep", "Go to school", "Sing"], correctAnswer: ["Wake up", "Have breakfast", "Go to school"], hint: "Morning activities" },
      { type: 'quiz', question: "Complete: 'She ___ to school at 8.'", options: ["go", "goes", "going", "is go", "Другой ответ"], correctAnswer: "goes", hint: "Present Simple with he/she/it" },
      { type: 'quiz', question: "What does 'do homework' mean?", options: ["Делать домашнее задание", "Идти в школу", "Смотреть телевизор", "Ложиться спать", "Другой ответ"], correctAnswer: "Делать домашнее задание", hint: "After school activity" }
    ],
    reward: { stars: 5, message: "Great! You know daily routine! ⏰" }
  },
  {
    title: "Days of the Week and Prepositions",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "Which preposition: '___ Monday'?", options: ["at", "on", "in", "to", "Другой ответ"], correctAnswer: "on", hint: "For days of the week" },
      { type: 'quiz', question: "Which preposition: '___ September'?", options: ["at", "on", "in", "by", "Другой ответ"], correctAnswer: "in", hint: "For months and seasons" },
      { type: 'quiz', question: "What does 'never' mean?", options: ["Всегда", "Обычно", "Иногда", "Никогда", "Другой ответ"], correctAnswer: "Никогда", hint: "0% frequency" },
      { type: 'quiz', question: "Where do adverbs of frequency go in a sentence?", options: ["After the verb", "Before the main verb", "At the end", "At the beginning", "Другой ответ"], correctAnswer: "Before the main verb", hint: "I always go to school" }
    ],
    reward: { stars: 5, message: "Well done! You know prepositions and days! 📅" }
  },
  {
    title: "Free Time Activities",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "Complete: 'I like ___ books.'", options: ["read", "reading", "to reading", "reads", "Другой ответ"], correctAnswer: "reading", hint: "like + -ing form" },
      { type: 'quiz', question: "How do you say 'играть на пианино'?", options: ["play piano", "play the piano", "play a piano", "playing piano", "Другой ответ"], correctAnswer: "play the piano", hint: "Use 'the' for musical instruments" },
      { type: 'quiz', question: "How do you say 'играть в футбол'?", options: ["play the football", "play football", "play a football", "play at football", "Другой ответ"], correctAnswer: "play football", hint: "NO article for sports" },
      { type: 'find', question: "Choose hobbies:", options: ["Reading", "Walking", "Swimming", "Sleeping", "Drawing", "Eating"], correctAnswer: ["Reading", "Swimming", "Drawing"], hint: "Activities people do for fun" }
    ],
    reward: { stars: 5, message: "Fantastic! You can talk about hobbies! 🎨⚽" }
  },
  {
    title: "Can / Can't for Abilities",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "Complete: 'She ___ play tennis.'", options: ["can", "cans", "can to", "is can", "Другой ответ"], correctAnswer: "can", hint: "Modal verb stays the same for all persons" },
      { type: 'quiz', question: "What does 'can't' mean?", options: ["Могу", "Не могу", "Хочу", "Не хочу", "Другой ответ"], correctAnswer: "Не могу", hint: "Negative ability" },
      { type: 'quiz', question: "How do you ask about someone's ability?", options: ["Does you can swim?", "Can you swim?", "Do you can swim?", "Are you can swim?", "Другой ответ"], correctAnswer: "Can you swim?", hint: "Put CAN at the beginning" },
      { type: 'match', question: "Match abilities with their meanings:", options: ["I can swim", "I can't drive", "Can I go out?", "I can cook"], correctAnswer: ["Я умею плавать", "Я не умею водить", "Можно мне выйти?", "Я умею готовить"], hint: "Ability, inability, and permission" }
    ],
    reward: { stars: 5, message: "Excellent! You know how to express abilities! 💪" }
  },
  {
    title: "At the Airport",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "How do you say 'паспорт' in English?", options: ["Ticket", "Passport", "Boarding pass", "Visa", "Другой ответ"], correctAnswer: "Passport", hint: "Travel document with your photo" },
      { type: 'find', question: "Choose airport places:", options: ["Gate", "School", "Check-in desk", "Hospital", "Customs", "Park"], correctAnswer: ["Gate", "Check-in desk", "Customs"], hint: "Places at an airport" },
      { type: 'quiz', question: "What does 'take off' mean?", options: ["Приземляться", "Взлетать", "Регистрироваться", "Садиться в самолёт", "Другой ответ"], correctAnswer: "Взлетать", hint: "When the plane leaves the ground" },
      { type: 'quiz', question: "Complete: 'I'd like a window ___.'", options: ["place", "seat", "chair", "spot", "Другой ответ"], correctAnswer: "seat", hint: "At the check-in desk" }
    ],
    reward: { stars: 5, message: "Excellent! You're ready to travel by plane! ✈️" }
  },
  {
    title: "Countries and Nationalities",
    subject: "Иностранный язык",
    icon: "Languages",
    color: "text-blue-400",
    tasks: [
      { type: 'quiz', question: "What nationality is from Japan?", options: ["Japanish", "Japanese", "Japan", "Japanian", "Другой ответ"], correctAnswer: "Japanese", hint: "People from Japan" },
      { type: 'quiz', question: "What is the capital of France?", options: ["London", "Berlin", "Paris", "Rome", "Другой ответ"], correctAnswer: "Paris", hint: "City of Light" },
      { type: 'quiz', question: "How do you ask about origin?", options: ["Where are you go?", "Where are you from?", "Where do you from?", "Where you from are?", "Другой ответ"], correctAnswer: "Where are you from?", hint: "be + from" },
      { type: 'quiz', question: "Which country needs 'the' before its name?", options: ["France", "Germany", "the USA", "China", "Другой ответ"], correctAnswer: "the USA", hint: "Countries with 'States' or 'Kingdom'" }
    ],
    reward: { stars: 5, message: "Wonderful! You know countries and nationalities! 🌍" }
  }
]

