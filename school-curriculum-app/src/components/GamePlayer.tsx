...
// Helper to shuffle array - deterministic version for SSR safety
const seededRandom = (seed: number): number => {
  const x = Math.sin(seed * 9301 + 49297) * 49297
  return x - Math.floor(x)
}

const shuffleArray = <T,>(arr: T[], seed: number = 42): T[] => {
  const result = [...arr]
...
export default function GamePlayer({ game, onBack, onComplete }: GamePlayerProps) {
  ...
  // Initialize ordered items in useEffect for SSR safety
  useEffect(() => {
    const firstTask = game.tasks[0]
    if (firstTask?.type === 'order' && firstTask.options) {
      setOrderedItems(shuffleArray(firstTask.options, 1))
    }
  }, [game.tasks])
  ...
  // Shuffled right column for match type - use seeded random
  const shuffledRightAnswers = useMemo(() => {
    const task = game.tasks[currentTask]
    if (task?.type === 'match' && Array.isArray(task.correctAnswer)) {
      return shuffleArray(task.correctAnswer as string[], currentTask * 7 + 3)
    }
    return []
  ...
  const nextTask = () => {
    if (currentTask < game.tasks.length - 1) {
      ...
      if (game.tasks[currentTask + 1].type === 'order' && game.tasks[currentTask + 1].options) {
        setOrderedItems(shuffleArray(game.tasks[currentTask + 1].options!, (currentTask + 1) * 13 + 7))
      }
    } else {
  ...
  const restartGame = () => {
    ...
    const firstTask = game.tasks[0]
    if (firstTask?.type === 'order' && firstTask.options) {
      setOrderedItems(shuffleArray(firstTask.options, 17))
    }
  }
