...
function AppContent() {
  ...
  // Mark as mounted on client (prevents hydration mismatch with Date)
  useEffect(() => {
    setMounted(true)
    setTodayStr(new Date().toLocaleDateString('ru-RU'))
  }, [])

  // During SSR and initial client hydration, render a simple placeholder
  // to avoid any hydration mismatch. Content appears after mount.
  if (!mounted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 p-4 md:p-8 relative overflow-hidden">
        <div className="max-w-7xl mx-auto relative z-10">
          <header className="text-center mb-6">
            ...
            <p className="text-lg text-purple-200">Загрузка...</p>
          </header>
        </div>
      </div>
    )
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <header className="text-center mb-6">
          ...
          <div className="flex justify-center items-center gap-2 mt-3 flex-wrap">
            ...
            {todayStr && <span className="text-yellow-400 text-xs bg-yellow-400/20 px-2 py-1 rounded-full">{todayStr}</span>}
