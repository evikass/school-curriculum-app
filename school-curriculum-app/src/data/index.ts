...
// Версия для cache-busting — обновляется при каждом деплое
const DATA_VERSION = Date.now().toString(36)

// Добавить basePath к относительным ссылкам на картинки
function fixImagePath(path: string, basePath: string): string {
...
function fixImagePaths<T>(data: T, basePath: string): T {
  if (Array.isArray(data)) {
    return data.map(item => fixImagePaths(item, basePath)) as T
  }
  if (data && typeof data === 'object') {
    ...
    for (const key in data) {
      ...
      if (key === 'image' && typeof value === 'string') {
        (result as Record<string, unknown>)[key] = fixImagePath(value, basePath)
      } else {
        (result as Record<string, unknown>)[key] = fixImagePaths(value, basePath)
      }
    }
    return result
  }
...
async function loadSubjectData(grade: number, subject: string): Promise<{ lessons?: SubjectData; games?: GameLesson[] }> {
  ...
  try {
    const url = `${basePath}/data/grades/${grade}/${subject}.json?v=${DATA_VERSION}`
    ...
    console.log(`[Data] Loaded ${grade}/${subject}: games=${data.games?.length || 0}`)
    // Исправляем пути к картинкам с учётом basePath
    return fixImagePaths(data, basePath)
  } catch (e) {
...
async function loadGradeBundle(grade: number): Promise<Record<string, { lessons?: SubjectData; games?: GameLesson[] }>> {
  ...
  const basePath = getBasePath()
  try {
    const url = `${basePath}/data/grades/${grade}/_bundle.json?v=${DATA_VERSION}`
    const response = await fetch(url, { cache: 'no-cache' })
    if (response.ok) {
      const data = await response.json()
      // Логируем загрузку для отладки
      const subjectKeys = Object.keys(data)
      const bioGames = data.biology?.games?.length || 0
      console.log(`[Data] Loaded bundle for grade ${grade}: ${subjectKeys.length} subjects, biology games=${bioGames}`)
      // Исправляем пути к картинкам с учётом basePath
      const fixed = fixImagePaths(data, basePath) as Record<string, { lessons?: SubjectData; games?: GameLesson[] }>
      bundleCache.set(grade, fixed)
      return fixed
    }
    // Если bundle не найден — fallback на загрузку отдельных файлов
    console.warn(`Bundle not found for grade ${grade} (status ${response.status}), falling back to individual files`)
  } catch (e) {
...
export async function getSubjectsForGrade(grade: number): Promise<Subject[]> {
  ...
  const subjects = keys
    .map(key => {
      const lessons = bundle[key]?.lessons
      if (lessons) {
        // Добавляем игры из bundle в предмет для отображения на карточке
        return { ...lessons, games: bundle[key]?.games || [] }
      }
      return null
    })
    .filter((s): s is SubjectData => !!s)

  console.log(`[Data] Grade ${grade}: ${subjects.length} subjects loaded, biology=${subjects.find(s => s.title === 'Биология')?.title || 'not found'}`)
